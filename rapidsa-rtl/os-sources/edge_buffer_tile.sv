...
module edge_buffer_tile #(
    ...
    //
    // The long inter-module route from MM2S (or the previous tile) terminates
    // at these registers, so all downstream logic (forwarding to next tile +
    // per-unit FIFO write) sees a clean local signal.
    //
    // These registers serve double duty: they ARE the 1-cycle forwarding
    // register to the next tile (m_data/m_valid/m_word_index are driven
    // combinationally from them), so chain forwarding latency is unchanged.
    // FIFO write latency is +1 cycle vs. the previous design — adjust the
    // FSM's chain-latency parameter accordingly.
    // =========================================================================
