// AXI DMA S2MM Controller - AXI-Lite Master for Xilinx AXI DMA IP
// Controls S2MM (Stream to Memory-Mapped) channel in direct register mode
// Note: Xilinx AXI DMA requires AWVALID and WVALID asserted simultaneously

module axi_dma_s2mm_ctrl #(
    ...
    // Control interface
    input  logic                    start,           // Start S2MM transfer
    ...
    // =========================================================================
    localparam OFFSET_S2MM_DMACR  = 12'h030;  // S2MM DMA Control
    localparam OFFSET_S2MM_DMASR  = 12'h034;  // S2MM DMA Status
    localparam OFFSET_S2MM_DA     = 12'h048;  // S2MM Destination Address
    localparam OFFSET_S2MM_DA_MSB = 12'h04C;  // S2MM Destination Address MSB
    localparam OFFSET_S2MM_LENGTH = 12'h058;  // S2MM Transfer Length
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (state)
                ST_SET_CTRL: begin
                    axi_addr     <= OFFSET_S2MM_DMACR;
                    axi_wdata    <= (1 << DMACR_RS);
                    return_state <= ST_SET_ADDR;
                end

                ST_SET_ADDR: begin
                    axi_addr     <= OFFSET_S2MM_DA;
                    axi_wdata    <= dst_addr_reg;
                    return_state <= ST_SET_LEN;
                end

                ST_SET_LEN: begin
                    axi_addr     <= OFFSET_S2MM_LENGTH;
                    axi_wdata    <= {9'b0, length_reg};
                    return_state <= ST_POLL;
                end

                ST_POLL: begin
                    axi_addr <= OFFSET_S2MM_DMASR;
